...
namespace limap
{
  ...
  double LineLinker2d::compute_score_overlap_oneway(const Line2d &l1,
                                                    const Line2d &l2) const
  {
    double overlap = compute_overlap<Line2d>(l1, l2);
    if (overlap > config.th_overlap)
      return 1.0;
    else
      return 0.0;
  ...
  bool LineLinker2d::check_connection_overlap_oneway(const Line2d &l1,
                                                     const Line2d &l2) const
  {
    return compute_score_overlap_oneway(l1, l2) == 1.0;
  }
  ...
  double LineLinker2d::compute_score_perp_oneway(const Line2d &l1, const Line2d &l2) const
  {
    double dist = compute_distance<Line2d>(l1, l2, LineDistType::PERPENDICULAR_ONEWAY);
    double score = expscore(dist, config.th_perp * config.multiplier());
    if (score < config.score_th)
      score = 0.0;
    return score;
  ...
  bool LineLinker2d::check_connection_perp_oneway(const Line2d &l1, const Line2d &l2) const
  {
    return compute_score_perp_oneway(l1, l2) >= config.score_th;
  }
  ...
  bool LineLinker2d::check_reprojection_score(const Line2d &l1, const Line2d &l2) const
  {
    // l1 is the detected 2D line and l2 is the projected 2D line from the 3D line
    if (config.use_angle)
      if (!check_connection_angle(l1, l2))
        return false;
    if (config.use_overlap)
      if (!check_connection_overlap_oneway(l2, l1))
        return false;
    if (config.use_perp)
      if (!check_connection_perp_oneway(l1, l2))
        return false;
    return true;
  }
  ...
  double LineLinker2d::compute_reprojection_score(const Line2d &l1, const Line2d &l2) const
  {
    // l1 is the detected 2D line and l2 is the projected 2D line from the 3D line
    double score = 1.0;
    if (config.use_angle)
    ...
    if (score < config.score_th)
      return score;
    if (config.use_overlap)
      score = std::min(score, compute_score_overlap_oneway(l2, l1));
    if (score < config.score_th)
      return score;
    if (config.use_perp)
      score = std::min(score, compute_score_perp_oneway(l1, l2));
    if (score < config.score_th)
      return score;
    return score;
  }
