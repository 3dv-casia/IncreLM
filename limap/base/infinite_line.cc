...
namespace limap
{
  ...
  Line3d GetLineSegmentFromInfiniteLine3d(const InfiniteLine3d &inf_line, const std::vector<CameraView> &views, const std::vector<Line2d> &line2ds, int num_outliers)
  {
  ...
  // Line3d GetLineSegmentFromInfiniteLine3d_linemap(const InfiniteLine3d &inf_line, const std::vector<CameraView> &views, const std::vector<Line2d> &line2ds, int num_outliers)
  // {
  //   THROW_CHECK_GT(line2ds.size(), 0);
  //   THROW_CHECK_EQ(views.size(), line2ds.size());
  //   int n_lines = line2ds.size();
  //   const V3D dir = inf_line.direction();
  //   const V3D p_ref = inf_line.point();

  //   if (n_lines < (num_outliers + 1))
  //   {
  //     num_outliers = n_lines - 1;
  //   }
  ...
  //   {
  //     const auto &view = views[i];
  //     const V3D C = view.pose.center();
  //     const Line2d &line2d = line2ds[i];
  //     InfiniteLine2d inf_line2d_proj = inf_line.projection(view);
  //     // project the two 2D endpoints to the 2d projection of the infinite line
  //     V2D pstart_2d = inf_line2d_proj.point_projection(line2d.start);
  //     V3D ray_start = view.ray_direction(pstart_2d);
  //     InfiniteLine3d line1(C, ray_start);
  //     V3D pstart_3d = inf_line.project_from_infinite_line(line1);
  //     double tstart = (pstart_3d - p_ref).dot(dir);

  //     V2D pend_2d = inf_line2d_proj.point_projection(line2d.end);
  //     V3D ray_end = view.ray_direction(pend_2d);
  //     InfiniteLine3d line2(C, ray_end);
  //     V3D pend_3d = inf_line.project_from_infinite_line(line2);
  //     double tend = (pend_3d - p_ref).dot(dir);
  ...
  Line3d GetLineSegmentFromInfiniteLine3d(const InfiniteLine3d &inf_line, const std::vector<Line3d> &line3ds, int num_outliers)
  {
