...
#undef CAMERA_MODEL_CASE
      ...
      std::map<int, LineTrack> HybridBAEngine::GetOutputLineTracks_linemap(const int num_outliers, const double var2d) const
      {
        std::map<int, LineTrack> outputs;
        for (auto it = line_tracks_.begin(); it != line_tracks_.end(); ++it)
        {
          ...
          views.reserve(track.image_id_list.size());
          for (size_t i = 0; i < track.image_id_list.size(); i++)
          {
            views.push_back(imagecols_.camview(track.image_id_list[i]));
          }

          Line3d line = GetLine3dFromInfiniteLine3d(lines_.at(track_id).GetInfiniteLine(), views, track.line2d_list, var2d, num_outliers);
