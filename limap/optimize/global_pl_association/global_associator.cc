    ...
    namespace global_pl_association
    {
      ...
      void GlobalAssociator::AddVPLineAssociationResiduals(const std::map<std::pair<int, int>, int> &weights)
      {
        ...
        for (auto it = weights.begin(); it != weights.end(); ++it)
        {
          ...
          // ceres::LossFunction *scaled_loss_function = new ceres::ScaledLoss(loss_function, double(weight) * config_.lw_vpline_association, ceres::DO_NOT_TAKE_OWNERSHIP);
          ceres::ResidualBlockId block_id = problem_->AddResidualBlock(cost_function, scaled_loss_function,
      ...
      void GlobalAssociator::AddVPOrthogonalityResiduals()
      {
        ...
        for (auto it = pairs.begin(); it != pairs.end(); ++it)
        {
          ...
          // ceres::LossFunction *scaled_loss_function = new ceres::ScaledLoss(loss_function, config_.lw_vp_orthogonality, ceres::DO_NOT_TAKE_OWNERSHIP);
          ceres::ResidualBlockId block_id = problem_->AddResidualBlock(cost_function, scaled_loss_function,
      ...
      void GlobalAssociator::AddVPCollinearityResiduals()
      {
        ...
        for (auto it = pairs.begin(); it != pairs.end(); ++it)
        {
          ...
          // ceres::LossFunction *scaled_loss_function = new ceres::ScaledLoss(loss_function, config_.lw_vp_collinearity, ceres::DO_NOT_TAKE_OWNERSHIP);
          ceres::ResidualBlockId block_id = problem_->AddResidualBlock(cost_function, scaled_loss_function,
                                                                       vps_.at(id1).data(), vps_.at(id2).data());
        }
      }
      ...
      std::map<std::pair<int, int>, int> GlobalAssociator::construct_weights_vpline(const int th_count) const
      {
        ...
        // construct weights
        std::map<std::pair<int, int>, int> weights; // ((vp3d_id, line3d_id), count)
