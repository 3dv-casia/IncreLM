...
def save_l3dpp_matches(folder, image_ids, matches_dir):
    if os.path.exists(folder):
        shutil.rmtree(folder)
    os.makedirs(folder)
    for img_id in tqdm(image_ids):
        # (ng_img_id, (N, 2))
        matches_tmp = read_npy(os.path.join(matches_dir, "matches_{0}.npy".format(img_id))).item()
        for ng_img_id, mat_matrix in matches_tmp.items():
            # delete possible repeated matches
            matches_dict = dict()
            for row in mat_matrix:
                ref_line2d_idx = row[0]
                matched_line2d_idx = row[1]
                if ref_line2d_idx not in matches_dict:
                    matches_dict[ref_line2d_idx] = set()
                matches_dict[ref_line2d_idx].add(matched_line2d_idx)
            matches_list = []
            for ref_line2d_idx, matched_line2d_indices in matches_dict.items():
                for matched_line2d_idx in matched_line2d_indices:
                    matches_list.append([ref_line2d_idx, matched_line2d_idx])
            # save l3dpp matches format
            fname = "matches_L3D++_{0}_{1}.txt".format(img_id, ng_img_id)
            fname = os.path.join(folder, fname)
            with open(fname, 'w') as f:
                n_matches = len(matches_list)
                f.write("{0}\n".format(n_matches))
                for match in matches_list:
                    # (ref_line2d_idx, matched_line2d_idx)
                    f.write("{0} {1}\n".format(match[0], match[1]))
            print("Writing line matches for L3DPP: {0}".format(fname))
    ...
    with open(fname, 'w') as f:
        ...
        for track_id in tqdm(range(n_tracks)):
            ...
            f.write("{0:.10f} {1:.10f} {2:.10f}\n".format(
                track.line.start[0], track.line.start[1], track.line.start[2]))
...
def read_lines_from_input(input_file):
    ...
    '''
    print(f"read lines from {input_file}", flush=True)
    ...
    # exception
    raise ValueError(
        "Error! File {0} not supported. should be txt, obj, or folder to the linetracks.".format(input_file))
