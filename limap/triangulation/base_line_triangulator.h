...
namespace limap
{
  ...
  namespace triangulation
  {
    ...
    public:
      ...
      void SetBipartites2d(const std::map<int, structures::PL_Bipartite2d> &all_bpt2ds)
      {
        use_pointsfm_ = true;
        all_bpt2ds_ = std::make_shared<std::map<int, structures::PL_Bipartite2d>>(all_bpt2ds);
      }
      void SetSfMPoints(const std::map<int, V3D> &points) { sfm_points_ = points; }

      // data
      std::vector<LineTrack> GetTracks() const { return tracks_; };
      std::vector<LineTrack> GetAllProposals() const { return all_proposals_; };
    ...
    protected:
      ...
      //
      std::vector<LineTrack> all_proposals_;
    };
