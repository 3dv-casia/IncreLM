...
#pragma omp parallel for
      ...
      for (int conn_id = 0; conn_id < n_conns; ++conn_id)
      {
        ...
        for (auto it = result.begin(); it != result.end(); ++it)
        {
          ...
          // //
          // LineTrack track;
          // track.line = std::get<0>(*it);
          // track.image_id_list.push_back(img_id);
          // track.line_id_list.push_back(line_id);
          // track.line2d_list.push_back(
          //     all_lines_2d_.at(img_id).at(line_id));
          // track.image_id_list.push_back(std::get<2>(*it).first);
          // track.line_id_list.push_back(std::get<2>(*it).second);
          // track.line2d_list.push_back(
          //     all_lines_2d_.at(std::get<2>(*it).first).at(std::get<2>(*it).second));
          // all_proposals_.push_back(track);
        }
      }
    }
