...
def line_localization(cfg, imagecols_db, imagecols_query, point_corresp, linemap_db, retrieval, results_path, img_name_dict=None, logger=None):
    ...
    if cfg['localization']['2d_matcher'] not in ['epipolar', 'sold2', 'superglue_endpoints', 'gluestick', 'linetr', 'lbd', 'l2d2']:
        raise ValueError("Unknown 2d line matcher: {}".format(
    ...
    if img_name_dict is None:
        img_name_dict = {img_id: imagecols_db.image_name(
    ...
    # GT for queries
    poses_db = {img_id: imagecols_db.camimage(
        img_id).pose for img_id in train_ids}
    ...
    if cfg['localization']['2d_matcher'] != 'epipolar':
        ...
        ma_cfg = {"method": matcher_name, "topk": 0, "n_jobs": cfg["n_jobs"], "superglue": {
        ...
        se_descinfo_dir = extractor.extract_all_images(
            folder_save, imagecols_db, all_db_segs, skip_exists=cfg['skip_exists'])
        se_descinfo_dir = extractor.extract_all_images(
            folder_save, imagecols_query, all_query_segs, skip_exists=True)

        basedir = os.path.join(
            "line_matchings", cfg["line2d"]["detector"]["method"], "feats_{0}".format(matcher_name))
        matcher = limap.line2d.get_matcher(
            ma_cfg, extractor, n_neighbors=cfg['n_neighbors_loc'], weight_path=weight_path)
        folder_save = os.path.join(cfg["dir_save"], basedir)
        retrieved_neighbors = {
            qid: [name_to_id[n] for n in retrieval[id_to_name[qid]]] for qid in query_ids}
        se_matches_dir = matcher.match_all_neighbors(
            folder_save, query_ids, retrieved_neighbors, se_descinfo_dir, skip_exists=cfg['skip_exists'])

    # Localization
    print("[LOG] Starting localization with points+lines...")
    final_poses = {}
    pose_dir = results_path.parent / \
        'poses_{}'.format(cfg['localization']['2d_matcher'])
    for qid in tqdm(query_ids):
        ...
        if cfg['localization']['2d_matcher'] != 'epipolar':
            ...
            all_line_pairs_2to2 = limapio.read_npy(os.path.join(
                se_matches_dir, "matches_{0}.npy".format(qid))).item()
        ...
        if cfg['localization']['reprojection_filter'] is not None:
            ...
            line_matches_2to3 = [
                (x, y) for x in all_line_pairs_2to3 for y in all_line_pairs_2to3[x]]
        ...
        final_pose, ransac_stats = _estimators.pl_estimate_absolute_pose(
            # query_pose not used for ransac methods
            cfg['localization'], l3ds, l3d_ids, l2ds, p3ds, p2ds, query_cam, query_pose,
        ...
        if cfg['localization']['skip_exists']:
            with open(os.path.join(pose_dir, f'{qid}.txt'), 'w') as f:
                ...
                line = ' '.join([name] + [str(x)
                                for x in fq] + [str(x) for x in ft]) + '\n'
                f.writelines([line])
    ...
    for qid in query_ids:
        ...
        line = ' '.join([name] + [str(x) for x in fq] + [str(x)
                        for x in ft]) + '\n'
        lines.append(line)
