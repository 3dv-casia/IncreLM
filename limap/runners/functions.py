...
def undistort_images(imagecols, output_dir, fname="image_collection_undistorted.npy", skip_exists=False, n_jobs=-1):
    ...
    if skip_exists:
        print("[LOG] Loading undistorted images (n_images = {0})...".format(
            imagecols.NumImages()))
    ...
    if n_jobs == -1:
        n_jobs = os.cpu_count()
    print("[LOG] Start undistorting images (n_images = {0})...".format(
        len(unload_ids)))
    ...
    def process(imagecols, img_id):
        ...
        imname_out = os.path.join(
            output_dir, "image{0:08d}.png".format(img_id))
    ...
    outputs = joblib.Parallel(n_jobs=n_jobs)(joblib.delayed(
        process)(imagecols, img_id) for img_id in tqdm(unload_ids))
    ...
    # save info
    limapio.save_txt_imname_dict(os.path.join(
        output_dir, 'original_image_list.txt'), imagecols.get_image_name_dict())
    ...
    if not cfg["load_meta"]:
        ...
        if not cfg["sfm"]["reuse"]:
            _psfm.run_colmap_sfm_with_known_poses(
                cfg["sfm"], imagecols, output_path=colmap_output_path, skip_exists=cfg["skip_exists"])
...
def compute_2d_segs(cfg, imagecols, compute_descinfo=True):  # -> tuple[dict, str | None]:
    """
    ...
    if "extractor" in cfg["line2d"]:
        print("[LOG] Start 2D line detection and description (detector = {0}, extractor = {1}, n_images = {2})...".format(
            cfg["line2d"]["detector"]["method"], cfg["line2d"]["extractor"]["method"], imagecols.NumImages()))
    else:
        print("[LOG] Start 2D line detection and description (detector = {0}, n_images = {1})...".format(
    ...
    detector = limap.line2d.get_detector(cfg["line2d"]["detector"], max_num_2d_segs=cfg["line2d"]["max_num_2d_segs"],
                                         do_merge_lines=cfg["line2d"]["do_merge_lines"], visualize=cfg["line2d"]["visualize"], weight_path=weight_path)
    if not cfg["load_det"]:
        if compute_descinfo and cfg["line2d"]["detector"]["method"] == cfg["line2d"]["extractor"]["method"] and (not cfg["line2d"]["do_merge_lines"]):
            all_2d_segs, descinfo_folder = detector.detect_and_extract_all_images(
    ...
    if cfg["line2d"]["save_l3dpp"]:
        limapio.save_l3dpp(os.path.join(
            folder_save, "l3dpp_format"), imagecols, all_2d_segs)
...
def compute_matches(cfg, descinfo_folder, image_ids, neighbors):
    ...
    import limap.line2d
    basedir = os.path.join("line_matchings", cfg["line2d"]["detector"]["method"], "feats_{0}".format(
        cfg["line2d"]["extractor"]["method"]))
    extractor = limap.line2d.get_extractor(
        cfg["line2d"]["extractor"], weight_path=weight_path)
    se_match = cfg["skip_exists"] or cfg["line2d"]["matcher"]["skip_exists"]
    matcher = limap.line2d.get_matcher(
        cfg["line2d"]["matcher"], extractor, n_neighbors=cfg["n_neighbors"], weight_path=weight_path)
    folder_save = os.path.join(cfg["dir_save"], basedir)
    ...
    if ("save_l3dpp_matches" in cfg["line2d"]) and cfg["line2d"]["save_l3dpp_matches"]:
        limapio.save_l3dpp_matches(os.path.join(
            folder_save, "l3dpp_matches_format"), image_ids, matches_folder)
...
def compute_exhausive_matches(cfg, descinfo_folder, image_ids):
    ...
    Returns:
        matches_folder (str): path to store the computed matches
    """
    print("[LOG] Start exhausive matching 2D lines... (extractor = {0}, matcher = {1}, n_images = {2})".format(
        cfg["line2d"]["extractor"]["method"], cfg["line2d"]["matcher"]["method"], len(image_ids)))
    import limap.line2d
    basedir = os.path.join("line_matchings", cfg["line2d"]["detector"]["method"], "feats_{0}".format(
        cfg["line2d"]["extractor"]["method"]))
