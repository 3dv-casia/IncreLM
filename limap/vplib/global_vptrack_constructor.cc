...
namespace limap
{
  ...
  namespace vplib
  {
    ...
    std::vector<VPTrack> GlobalVPTrackConstructor::ClusterLineTracks(const std::vector<LineTrack> &linetracks, const ImageCollection &imagecols) const
    {
      ...
      std::sort(vptracks.begin(), vptracks.end(), [](const auto &d1, const auto &d2)
                { return d1.length() > d2.length(); });
      return vptracks;
    }

    // std::vector<VPTrack> GlobalVPTrackConstructor::ClusterLineTracks(const std::map<int, LineTrack> &linetracks, const ImageCollection &imagecols) const
    // {
    //   // count edges
    //   std::map<std::pair<Node2d, Node2d>, int> edge_counters;
    //   for (const auto& pair : linetracks)
    //   {
