...
def parse_config():
    ...
    arg_parser.add_argument('-c', '--config_file', type=str,
                            default='cfgs_linemap/localization/7scenes.yaml', help='config file')
    arg_parser.add_argument('--default_config_file', type=str,
                            default='cfgs_linemap/localization/default.yaml', help='default config file')
    arg_parser.add_argument('--dataset', type=Path,
                            required=True, help='7scenes root path')
    arg_parser.add_argument('-s', '--scene', type=str,
                            required=True, help='scene name(s)')
    ...
    arg_parser.add_argument('--query_images', default=None,
                            type=Path, help='Path to the file listing query images')
    arg_parser.add_argument('--eval', default=None,
                            type=Path, help='Path to the result file')
...
def main():
    ...
    # Output path for LIMAP/IncreLM results (tmp)
    if cfg['output_dir'] is None:
        cfg['output_dir'] = 'tmp/7scenes/{}'.format(args.scene)
    # Output folder for LIMAP/IncreLM linetracks (in tmp)
    ...
    # Some paths useful for LIMAP/IncreLM localization too
    ...
    retrieval_path = args.dataset / \
        '7scenes_densevlad_retrieval_top_10' / f'{args.scene}_top10.txt'

    ##########################################################
    # [B] LIMAP/IncreLM triangulation/fitnmerge for database line tracks
    ##########################################################
    ...
    if not args.use_dense_depth:
        ...
        if not cfg['skip_exists'] or not os.path.exists(finaltracks_dir):
            ...
            print('Running IncreLM...')
            _, _, linetracks_db = _runners_linemap.line_triangulation(
                cfg, imagecols_train, neighbors=neighbors, ranges=ranges)
        else:
            linetracks_db = limapio.read_folder_linetracks(finaltracks_dir)
            logger.info(f'Loaded IncreLM triangulation result from {finaltracks_dir}')
    else:
        finaltracks_dir = os.path.join(cfg['output_dir'], cfg['output_folder'])
        if not cfg['skip_exists'] or not os.path.exists(finaltracks_dir) or not cfg['merging']['do_merging']:
            ...
            depths = {id: DepthReader(image_path_to_rendered_depth_path(
                all_images[id].name), depth_dir) for id in train_ids}
    ...
    for qid in query_ids:
        ...
        point_correspondences[qid] = {
            'p2ds': p2ds, 'p3ds': p3ds, 'inliers': inliers}
